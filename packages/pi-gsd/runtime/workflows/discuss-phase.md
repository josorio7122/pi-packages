...
```bash
Use the gsd_util tool with action: ""docs(${padded_phase}"): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
Use the gsd_util tool with action: ""docs(state"): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
<objective>
    ...
    <execution_context>
    Read workflow: $GSD_RUNTIME_PATH/workflows/plan-phase.md
