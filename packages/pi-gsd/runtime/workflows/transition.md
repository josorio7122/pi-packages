...
```bash
TRANSITION=$(Use the gsd_phase tool with action: "complete", args: "${current_phase}")
```
...
```bash
PROGRESS=$(Use the gsd_util tool with action: "bar", args: --raw)
```
...
```bash
Use the gsd_config tool with action: "set", args: workflow.auto_advance false
```
