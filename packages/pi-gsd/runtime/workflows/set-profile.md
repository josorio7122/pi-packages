...
```bash
Use the gsd_config tool with action: "ensure-section"
INIT=$(Use the gsd_state tool with action: "load")
```
...
[Show table from MODEL_PROFILES using the gsd_config tool with action: "resolve-model" for selected profile]
