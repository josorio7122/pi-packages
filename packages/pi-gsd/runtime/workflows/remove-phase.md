...
```bash
INIT=$(Use the gsd_init tool with action: "phase-op", args: "${target}")
```
...
```bash
RESULT=$(Use the gsd_phase tool with action: "remove", args: "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(Use the gsd_phase tool with action: "remove", args: "${target}" --force)
```
...
```bash
Use the gsd_util tool with action: ""chore:", args: remove phase {target} ({original-phase-name})" --files .planning/
```
