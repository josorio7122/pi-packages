...
```bash
INIT=$(Use the gsd_init tool with action: "milestone-op")
```
...
```bash
integration_checker_model=$(Use the gsd_util tool with action: "resolve-model", args: gsd-integration-checker --raw)
```
...
Use the gsd_phase tool with action: "list"
```
...
PHASE_INFO=$(Use the gsd_phase tool with action: "find-phase", args: 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  Use the gsd_util tool with action: "summary-extract", args: "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
