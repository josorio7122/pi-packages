...
```bash
ROADMAP=$(Use the gsd_roadmap tool with action: "analyze")
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  Use the gsd_util tool with action: "summary-extract", args: "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(Use the gsd_milestone tool with action: "complete", args: "v[X.Y]" --name "[Milestone Name]")
```
...
**If doesn't exist:** Create from template at `$GSD_RUNTIME_PATH/templates/retrospective.md`.
...
**Commit:**
```bash
Use the gsd_util tool with action: ""docs:", args: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(Use the gsd_init tool with action: "execute-phase", args: "1")
```
...
```bash
Use the gsd_util tool with action: ""chore:", args: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
