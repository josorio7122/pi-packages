...
Read reference: $GSD_RUNTIME_PATH/references/ui-brand.md
</required_reading>
...
```bash
INIT=$(Use the gsd_init tool with action: "plan-phase", args: "$PHASE")
```
...
5. Commit:
```bash
Use the gsd_util tool with action: ""docs(${padded_phase}"): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(Use the gsd_roadmap tool with action: "get-phase", args: "${PHASE}" | jq -r '.section')
```
...
gsd_dispatch(
  ...
  task: research_prompt  // See prompt block above
)
```
...
**If found:**
1. Read validation template from `$GSD_RUNTIME_PATH/templates/VALIDATION.md`
...
```bash
Use the gsd_util tool with action: "commit-docs", args: "docs(phase-${PHASE}): add validation strategy"
```
...
gsd_dispatch(
  ...
  model: "{planner_model}",
  // Plan Phase {phase}
  task: filled_prompt  // See prompt block above
)
```
...
gsd_dispatch(
  agent: "general-purpose",
  model: "{planner_model}",
  // Revise Phase {phase} plans
  task: revision_prompt  // See prompt block above
)
```
...
<objective>
    ...
    <execution_context>
    Read workflow: $GSD_RUNTIME_PATH/workflows/execute-phase.md
