...
```bash
INIT=$(Use the gsd_init tool with action: "quick", args: "$DESCRIPTION")
```
...
**Agent failure handling:** If executor reports "failed" but the summary file exists and git log shows commits, treat as successful — the failure may be a transient reporting issue.
...
```bash
Use the gsd_util tool with action: ""docs(quick-${next_num}"): ${DESCRIPTION}" --files ${file_list}
```
