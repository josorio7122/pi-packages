...
Read reference: $GSD_RUNTIME_PATH/references/git-integration.md
</required_reading>
...
```bash
INIT=$(Use the gsd_init tool with action: "execute-phase", args: "${PHASE}")
```
...
**Pattern A:** init_agent_tracking → spawn `gsd_dispatch(agent: "gsd-executor", task: "execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash")` → track agent_id → wait → update tracking → report.
...
3. After ALL segments: aggregate files/deviations/decisions → create SUMMARY.md → commit → self-check:
   ...
   **Agent failure handling:** If any segment agent reports "failed" but spot-checks pass (summary exists, commits present), treat as successful — the failure may be a transient reporting issue.
...
```bash
Use the gsd_phase tool with action: "list", args: --type summaries --raw
...
See `$GSD_RUNTIME_PATH/references/tdd.md` for structure.
</tdd_plan_execution>
...
See $GSD_RUNTIME_PATH/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `$GSD_RUNTIME_PATH/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `$GSD_RUNTIME_PATH/templates/summary.md`.
...
Use the gsd_state tool with action: "advance-plan"

# Recalculate progress bar from disk state
Use the gsd_state tool with action: "update-progress"

# Record execution metrics
Use the gsd_state tool with action: "record-metric", args: \
...
Use the gsd_state tool with action: "add-decision", args: \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
Use the gsd_state tool with action: "add-blocker", args: --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
Use the gsd_state tool with action: "record-session", args: \
...
```bash
Use the gsd_roadmap tool with action: "update-plan-progress", args: "${PHASE}"
```
...
```bash
Use the gsd_requirements tool with args: mark-complete ${REQ_IDS}
```
...
```bash
Use the gsd_util tool with action: ""docs({phase}-{plan}"): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
Use the gsd_util tool with action: """", args: --files .planning/codebase/*.md --amend
```
</step>
