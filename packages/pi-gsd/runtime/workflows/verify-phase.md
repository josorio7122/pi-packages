...
<required_reading>
Read reference: $GSD_RUNTIME_PATH/references/verification-patterns.md
Read template: $GSD_RUNTIME_PATH/templates/verification-report.md
</required_reading>
...
```bash
INIT=$(Use the gsd_init tool with action: "phase-op", args: "${PHASE_ARG}")
```
...
```bash
Use the gsd_roadmap tool with action: "get-phase", args: "${phase_number}"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(Use the gsd_util tool with action: "get", args: "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(Use the gsd_roadmap tool with action: "get-phase", args: "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(Use the gsd_verify tool with action: "artifacts", args: "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(Use the gsd_verify tool with action: "key-links", args: "$plan")
...
See $GSD_RUNTIME_PATH/templates/verification-report.md for complete template.
</step>
