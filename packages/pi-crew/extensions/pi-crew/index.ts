...
export default function piCrew(pi: any) {
	...
	pi.registerTool({
		...
		async execute(
			...
			if (modeCount !== 1) {
				return {
					content: [{ type: "text", text: "Error: Specify exactly one mode — single (preset + task), parallel (tasks array), or chain (chain array)." }],
					isError: true,
				};
			}
			...
			): { resolved: SpawnParams | null; error: string | null } => {
				const resolved = resolvePreset(presetName, profile, overrides, packageRoot);
				if (!resolved) {
					return { resolved: null, error: `Unknown preset "${presetName}". Available: ${getPresetNames().join(", ")}` };
				}
				return {
					error: null,
					resolved: {
			...
			if (hasSingle) {
				...
				const agents = buildAgentStates([{ preset: params.preset, task: params.task, model: r.resolved!.model }]);
				...
				try {
					...
					agents[0].elapsedMs = Date.now() - startTime;
					return {
						content: [{ type: "text", text: `Agent aborted: ${e.message}` }],
						details: { mode: "single", agents } as CrewDispatchDetails,
						isError: true,
					};
				}
			...
			if (hasTasks) {
				...
				try {
					...
					for (const agent of agents) {
						...
						const label = agent.preset.charAt(0).toUpperCase() + agent.preset.slice(1) +
							(showNum ? ` #${agent.instance}` : "");
						const prefix = agent.status === "error" ? "[ERROR] " : "";
						const agentOutput = getFinalOutput(agent.messages) || "(no output)";
			...
			if (hasChain) {
				...
				for (let i = 0; i < params.chain.length; i++) {
					...
					try {
						...
						agents[i].elapsedMs = Date.now() - startTime;
						return {
							content: [{ type: "text", text: `Chain aborted at step ${i + 1}: ${e.message}` }],
							details: { mode: "chain", agents } as CrewDispatchDetails,
							isError: true,
						};
					}
			...
			return { content: [{ type: "text", text: "Internal error: no mode matched." }], isError: true };
		},
		...
		renderResult(result: any, options: { expanded: boolean; isPartial: boolean }, theme: any) {
			const details = result.details as CrewDispatchDetails | undefined;
	...
	pi.registerCommand("crew:override", {
		...
		handler: async (args: string, ctx: any) => {
			...
			writeConfig(ctx.cwd, config);
			ctx.ui.notify(`Override set: ${agentName} → ${model}`, "success");
		},
	});
	...
	pi.registerCommand("crew:reset", {
		...
		handler: async (_args: string, ctx: any) => {
			...
			if (fs.existsSync(statePath)) {
				...
				ctx.ui.notify("State cleared. Starting fresh.", "success");
			} else {
